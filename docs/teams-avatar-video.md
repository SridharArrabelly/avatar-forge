...
### Option 1 — WebSocket avatar mode (`response.video.delta`)  ✅ preferred

Voice Live already supports a **websocket** avatar output mode (`avatarOutputMode =
"websocket"`) in which it streams the rendered avatar video to the client as
`response.video.delta` events — **base64 H264 chunks over the same Voice Live WebSocket**.
The repo already handles these today for the browser (`event_handlers.py` →
`{"type":"video_data","delta":…}`). Crucially, **no WebRTC/SDP/ICE handshake is
involved** — the video arrives in-band on the socket the bridge already holds.

For Slice 2A the bridge would:

1. Configure its Voice Live session with `avatarEnabled=true`,
   `avatarOutputMode="websocket"`, `codec="h264"` (reusing `build_avatar_config`).
2. Receive `response.video.delta` chunks, accumulate the H264 elementary stream.
3. **Decode H264 → raw frames** with PyAV/ffmpeg (`av.CodecContext` / `VideoFrame`).
   H264 decodes to YUV420p, which converts to **NV12** by interleaving the U and V
   planes — cheap, no colour-space maths.
4. Forward each decoded NV12 frame as a `VideoData` bridge frame (base64 + width/height).

**Why preferred:** it reuses an existing, in-repo, browser-proven path; it avoids the
fragile headless-WebRTC stack; and decode-to-NV12 is a standard, well-supported pipeline.
The cost is a software H264 decode per frame (manageable at 360p/15fps; see §7).
...
### 5.2 Python backend (`backend/`) — the remaining increment

Additive and gated (e.g. a `MEETING_BOT_VIDEO` / config flag) so a non-video deploy is
unchanged:

- **`backend/acs/bridge.py`** — when video is enabled, configure the bridge's Voice Live
  session with the avatar on in **websocket** mode; consume `response.video.delta`;
  decode H264 → NV12 (PyAV); send `VideoData` frames down the bot socket. Reuse the
  existing outbound-gating / barge-in so video stops when audio stops.
- **`backend/voice/`** — **unchanged**; `build_avatar_config` already produces the avatar
  config and the event layer already surfaces `response.video.delta`. We only flip the
  flags for the bridge session.
- New dependency: **PyAV** (ffmpeg bindings) for H264 decode — pin it; it ships manylinux
  wheels so the Linux ACA image is fine (decode runs on the Python side, which is *not*
  the Windows-locked part).
