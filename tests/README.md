...
| [`test_preflight.py`](test_preflight.py) | The helpers that settle subscription / location / resource group — including the no-TTY cases, where a stray prompt would hang a deploy forever with no visible question. Also the agent web-tool check, that an unset `AGENT_WEB_TOOL` is recorded as Web IQ on a new environment but pinned to Bing on an already-deployed or BYO-Foundry one, and that its auth settle creates or reuses the app registration, falls back to a fresh key on any directory refusal, and never raises. |
...
| [`test_agent_prompt.py`](test_agent_prompt.py) | The active concise agent prompt resolves all four placeholders, expands onboarding once, stays bounded in size and retains explicit routing/output/accuracy authoring guards. These are static contracts, not live model-quality tests. |
...
| [`test_agent_tool_wiring.py`](test_agent_tool_wiring.py) | That a missing **optional** tool degrades gracefully while a missing **required** one fails loudly. Includes the Web IQ OpenAPI tool in key and managed-identity modes, and that its names match the wrapper route. |
| [`test_agent_web_tool.py`](test_agent_web_tool.py) | The agent's `/api/tools/search-web` route: 404 when not configured, constant-time key check, and for Entra tokens the signature, audience, issuer, expiry, tenant **and** caller `oid` — a real token for the right audience from the wrong identity is refused. |
| [`test_agent_web_tool_infra.py`](test_agent_web_tool_infra.py) | `AGENT_WEB_TOOL` in the generated ARM: Bing and Web IQ are mutually exclusive, the container gets Web IQ settings and the site list only when the tool needs them, the key is a secret everywhere and wins over the audience, and no output echoes an input. Run `az bicep build --file infra\main.bicep` first. |
...
| [`test_set_profile.py`](test_set_profile.py) | That profile flags are authoritative rather than cumulative, so switching profiles clears the previous one's flags instead of leaving you paying for its resources. Also that the web-tool menu defaults to Web IQ, and that an environment deployed before that default keeps Bing. |
...
| [`test_sync_via_mirror.py`](test_sync_via_mirror.py) | That [`sync_via_mirror.py`](../scripts/sync_via_mirror.py) hands the mirror only to `uv pip` — never to `uv export`/`uv sync`, which would rewrite `uv.lock` with mirror URLs and break the image build — finds pip's mirror in the right precedence, and never prints credentials. Its git hooks: run only on events that can change the tree, try offline first, fall back to the mirror (or a plain `uv sync` with no mirror), keep installed extras, never remove packages or fail git, install idempotently with LF endings, and leave other people's hooks alone. Also that the project builds with `uv_build`, so a `pyproject.toml` edit needs no download. |
